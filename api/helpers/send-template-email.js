...
Instead, just log it to the console.`,
    ...
    smtpMail: {
      description: 'The smtp email id for sending mail.',
       example: 'foo@bar.com',
      required: true
    },
    smtpPassword: {
      description: 'The smtp password for sending mail.',
...
> from the terminal output into your browser.)
 ...
 [?] If you're unsure, visit https://sailsjs.com/support`
      ...
      let transporter = nodemailer.createTransport({
        // host: sails.config.custom.mailhost,
        // port: sails.config.custom.mailport,
        // secure: sails.config.custom.mailsecure, // secure:true for port 465, secure:false for port 587
        service: 'gmail',
        auth: {
            user: inputs.smtpMail,
            pass: inputs.smtpPassword
        }
      });
      let mailOptions = {
        from: `TVH Requirement Panel`, // sender address
    ...
    return exits.success({
      loggedInsteadOfSending: inputs.to
    });
