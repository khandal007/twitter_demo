(function (document) {
	...
	if($("#stockSiteMode").length > 0){
     ...
     $("#invoiceform").submit(function(e){
         ...
         /*if(formData.name == "custom_label[]" || formData.name == "custom_value[]"){
         	var other_info = {};
         	for(i=0;i<custom_label.length;i++){
         		other_info[custom_label[i]] = custom_value[i];
         	}
         }*/
...
//												console.log(element.account_category);
			...
			onChooseEvent: function() {
				...
				//console.log("Here");
				//console.log(element);
				  doAjaxCall('/getProjects',{cust_id:element.id},"GET",false,'',function(data,type){
		            if(type == 1){
		              //  $("#prevHeadsDiv").show();
		             
		                    var str = "<option value=''> Select Project</option>";
		                    data.project.map(function(res){
		                    str =`${str}<option value="${res.id}" data-podate="${res.transaction_for_date}">${res.project_name}</option>`;
		                    });
		                    $("#projectId").html(str);
		                    $("#projectId").select2();
...
function updateProductRow(event, data, roundType){
	...
	//taxTotal = total+cgstTotal+sgstTotal+igstTotal+expenseVal;
...
function addRow(){
	var str = `<tr>
					<th><input type="text" name="custom_label[]" value="" placeholder="custom_label" required></th>
					<td colspan="3"><input type="text" placeholder="custom_value" style="width:95%" class="" name="custom_value[]" value="" required> &nbsp;
