$(document).ready(function(e){
        ...
        $("#receiptForm").submit(function(e){
                ...
                if($("#paymentMode").val() == 1){
                    txt = `Are you sure you want to receive Rs ${amountSend}/- from ${headPerson} through cash`;
                }else if($("#paymentMode").val() == 2){
                    txt = `Are you sure you want to receive Rs ${amountSend}/- from ${headPerson} through cheque`;
                }else{
                    txt = `Are you sure you want to receive Rs ${amountSend}/- from ${headPerson} through bank transfer`;
                }
                var r = confirm(txt);
                if (r == true) {
                $("#trLoader").show();
                ...
                let transactionData = $("#receiptForm").serialize();
                 $.ajax({
                    type: 'post',
                    url: '/master/savetransaction?type=1',
    ...
    function payMode(obj){
        var val = parseInt($(obj).val());
        // $('option:selected', this).attr('mytag');
        $("#page_mode").val($('option:selected', obj).attr('data-mode'));
        if(val == 2){
            $(".cheque_no").show("slow");
