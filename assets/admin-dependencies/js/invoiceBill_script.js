io.socket.on('updateTransactionList', function (data){
   // alert(111);
      console.log(data);
      location.replace("/revenueList?type=3");
    });
(function (document) {
	$("#siteVal").select2();
	...
	/*if($("#stockSiteMode").length > 0){
		$("#stockSiteMode").change(function(e){
			//alert(111);
			var debitHead= parseInt($("#debit_head").val());
			if($(this).val() != ""){
				  ...
				  doAjaxCall('/servicetype',{value:$(this).val(),mode:$("#invoiceMode").val()},"GET",false,'',function(data,type){
		            if(type == 1){
		              //  $("#prevHeadsDiv").show();
		                    ...
		                    str =`${str}<option value="${res.id}" ${sel}>${res.account_name} (${res.balance})</option>`;
		                    sel = "";
		                    });
		                    $("#drHeadVal").html(str);
		...
		if($("#gsType").val() != 0){
			 var changeVal = $("#gsType").val();
			 $('#stockSiteMode').val(changeVal).trigger('change');
		}
     ...
     $("#invoiceform").submit(function(e){
         ...
         var formData1 = $("#invoiceform").serialize();
         //console.log(formData);
          var sel = $('.invCat:checked').map(function(_, el) {
                return $(el).val();
            }).get();
         formData.map(function(d, i){
         	if(d.name == "custom_label[]"){
         		if(allData['formData'].hasOwnProperty("custom_obj")){
         			allData['formData']["custom_obj"][d.value] = formData[i+1].value;
         		}else{
         			allData['formData']["custom_obj"] = {};
         			allData['formData']["custom_obj"][d.value] = formData[i+1].value;
         		}
         	}
         	if(d.name == "siteVal"){
         ...
         var cells, displayCells,productIds, totalCells, a, i,gstVal,goodSiteVal;
         var allProducts = [];
         var totalObj = {};
         ...
         for (var a = document.querySelectorAll('table.inventory tbody tr'), i = 0; a[i]; ++i) {
			...
			productObj["revenue_goods"] = parseInt(productIds[0].value);
		...
		/* if(totalObj["transaction_roundoff"] != 0 ){
		 ...
		 if(parseFloat($("#totalInvAmount").val()) != totalObj["transaction_basic"]){
        ...
        switch(parseInt($("#invoiceMode").val())) {
     		...
     		case 9:
     			//REVENUE
     			var accName = $("#headName").val();
     			var str = `Are you sure you want to create REVENUE of amount ${parseFloat(totalObj.transaction_amount)}/- for ${accName} ?`;
     			break;		
     		default:
     			break;	
     	}
	...
	onSelectItemEvent: function() {
	 ...
	 console.log(element);
		if(element.id){
			...
			$("#accountStateId").val(element.state_id); 
			$("#accountWorkType").val(element.account_work_type);
			if(element.sell_slug){
								$("#sell_slug").val(element.sell_slug);
							}else{
								$("#sell_slug").val('');
							}
			//$("#invoiceCategoryText").text(element.account_work_type_text);
//												console.log(element.account_category);
			if($("#invoiceMode").val() == 9){
				if(element.account_category != 3){
				if($("#accountStateId").val() == $("#companyStateId").val()){
				//alert("Same State");
				$("#invoiceTypeText").text("CGST/SGST");
...
$("#headVal").easyAutocomplete(options);
	...
	function getAllProjects(customer_id){
		doAjaxCall('/getProjects',{cust_id:customer_id},"GET",false,'',function(data,type){
        if(type == 1){
          //  $("#prevHeadsDiv").show();
           var selectedProject  = $("#projectId").attr('data-selected');
            var str = "<option value=''> Select Project</option>";
            data.project.map(function(res){
            	var sel ="";
            	if(selectedProject == res.id){
            		sel = "selected";
            	}
            str =`${str}<option value="${res.id}" ${sel} data-podate="${res.transaction_for_date}">${res.project_name}</option>`;
            sel="";
            });
            $("#projectId").html(str);
            $("#projectId").select2();
...
function updateProductRow(event, data, roundType){
	...
	taxTotal = parseFloat(roundToFixed(total))+parseFloat(roundToFixed(cgstTotal))+parseFloat(roundToFixed(sgstTotal))+parseFloat(roundToFixed(igstTotal));
	// get balance cells
...
function checkFinancialYear(checkDay){
	...
	$("#trLoader").show();
	var allData = {mode:$("#invoiceMode").val(),checkDate:checkDay,pageMode:$("#pageMode").val(),sell_slug:$("#sell_slug").val(),head_id:$("#headId").val()}	
	  $.ajax({
        type: 'GET',
        url: '/getRevenueSerialNumber',
        data: allData,
        datatype: 'json',
        success: function (data) {
            if(data.status == "success"){
...
function onContentLoad() {
	 ...
	 //updateInvoice();
	 //Invoice Date
	 if($('#mode_date').length > 0){
	 	 var getTxnDate = $("#mode_date").attr('data-date');
	 	 var revstart = $("#mode_date").attr('data-revstart');
	 	 var revend = $("#mode_date").attr('data-revend');
	 	 if(getTxnDate != ""){
 	
	 	 	$('#mode_date').val(getTxnDate);
	 	 	$('#mode_date').datepicker({ format : 'dd/mm/yyyy', autoclose: true });
	 	 	if(revstart != ""){
		 	 	$('#mode_date').datepicker('setStartDate', new Date(revstart));
	 	 	}
	 	 	if(revend != ""){
		 	 	$('#mode_date').datepicker('setEndDate', new Date(revend));
	 	 	}else{
	 	 		$('#mode_date').datepicker('setEndDate', new Date());
	 	 	}
	 	 	$('#mode_date').datepicker('update', new Date(getTxnDate));
	 	 	// $('#mode_date').datepicker('disable');
	 	 }else{
	 	 $('#mode_date').datepicker({ format : 'dd/mm/yyyy', autoclose: true }); 	 	 
	 ...
	 if($('#po_date').length > 0){
	 	var getPoDate = $("#po_date").attr('data-date');
	 	 $('#po_date').datepicker({ format : 'dd/mm/yyyy', autoclose: true }); 
	 	if(getPoDate != ""){
	 		var setPoDate = new Date(getPoDate);
	 		$('#po_date').datepicker('update', setPoDate);
	 	}
	 	 
	 } 
	 if($('#inv_date').length > 0){
	 	var getInvDate = $("#inv_date").attr('data-date');
	 	 $('#inv_date').datepicker({ format : 'dd/mm/yyyy', autoclose: true }); 
	 	if(getInvDate != ""){
	 		var setInvDate = new Date(getInvDate);
	 	}else{
	 		var setInvDate = new Date();
	 	}
	 	 $('#inv_date').datepicker('update', setInvDate);
	 } 
	 if($('#challan_date').length > 0){
	 	var chl_date = $("#challan_date").attr('data-date');
	 	$('#challan_date').datepicker({ format : 'dd/mm/yyyy', autoclose: true }); 
	 	if(chl_date != ""){
	 		$('#challan_date').datepicker('update', new Date(chl_date));
	 	}
	 }
	 if($('#gate_date').length > 0){
	 	var gate_entry_date = $("#gate_date").attr('data-date');
	 	$('#gate_date').datepicker({ format : 'dd/mm/yyyy', autoclose: true }); 
	 	if(gate_entry_date != ""){
	 		$('#gate_date').datepicker('update', new Date(gate_entry_date));
	 	}
	 }
	var
	...
	if($("#editMode").val() != 0){
		$("#btnSave").text("Edit");
		changeAllCostCenters();
		getAllProjects($("#headId").val());
		for(var i = $("#totalRows2").val();i>0;i--){
			initializeGoods(i);
		}
	}
...
function generateTableRow2(id){
  var totalCount = $("#" + id).val();
  var rowsCount = parseInt(totalCount)+1;
  var projectId = $("#projectId").val();
  var data = {rowsCount:rowsCount,project_id:(projectId)?projectId:null};
  doAjaxHTMLCall('/getInvoicePartial',data,"GET",true,'loaderOverlay',function(data){
      var emptyColumn = document.createElement('tr');
...
function initializeGoods(rowsCount, ){
	var productOptions = {
	url: function(phrase){ return "/getGoodsList"; },
	getValue: function(element){ 
			return element.label; 
	},    
	list: {
	...
	ajaxSettings: {
			dataType: "json",
			method: "GET",
			data: { dataType: "json", customer_id: $("#headId").val() }
	},
	preparePostData: function(data) {
...
function getRevenueItem(obj){
	console.log("obj",$(obj).val());
	$("#revenueReq").empty();
	$.ajax({
	    type: "GET",
	    url: '/getRevenueRequest',
	    data: {po_id: $(obj).val()},
	    datatype:'json',
	...
	doAjaxCall('/getProjects',{po_id:$(obj).val()},"GET",false,'',function(data,type){
		
        var str = "<option value=''> Cost Center</option>";
        data.costCenter.map(function(res){
        str =`${str}<option selected value="${res.site_id.id}">${res.site_id.site_name}</option>`;
        });
        $("#siteVal").html(str);
        $("#siteVal").select2();
    });
...
function getPoNumber(obj){
	var val = $(obj).val();
if(val != ""){
	doAjaxCall('/getPoNumber',{projectId:val},"GET",false,'',function(data,type){
		
        var str = "<option value=''> Select PO</option>";
        data.po.map(function(res){
        str =`${str}<option value="${res.id}">${res.revenue_for_serial}(${res.revenue_for_date})</option>`;
        });
        $("#poNumber").html(str);
    });
}        
...
function addRow(){
	var str = `<tr>
		<th><input type="text" name="custom_label[]" value="" placeholder="Custom Label" required></th>
		<td colspan="3"><input type="text" placeholder="Custom Value" style="width:95%" class="" name="custom_value[]" value="" required> &nbsp;

		<a>
          <i class="fa fa-minus" id="" onclick="deleteRow(this)"></i>
        </a>
		</td>
	</tr>`;
...
function getSites(){
    ...
    var rows = parseInt($("#totalRows2").val())+1;
      if(val != ""){
        doAjaxCall('/getSiteListByProject',{project_id:val},"GET",false,'',function(data,type){
      	for(var i=1; i<=rows;i++){
            if(type == 1){
                ...
                $("#catselect_"+i).html(str);
            }
    	}
        });
                  
        }else{
          var str = "<option value=''> No Costcenter found</option>";
         $("#catselect_"+i).html(str);

        }
}
function uploadExcel(){
  var file = $('#excel_document')[0].files[0];
  var ajaxData = new FormData();
  //alert(111);
  if(($("#headId").val() == "") && ($("#gstType").val() == 0) && $("#site_id").val() == ""){
  	alert("No customer or Costcenter choosen");
  	return false;

  }
  $("#trLoader").show();
  ajaxData.append('document',file);
  ajaxData.append('customer_id',$("#headId").val());
  ajaxData.append('siteVal',$("#siteVal").val());
  ajaxData.append('projectVal',$("#projectId").val());
  ajaxData.append('siteValName', $("#siteVal option:selected").html());
  ajaxData.append('tax_type',$("#gstType").val());
  $.ajax({
	  type: "POST",
	  url: "/uploadInstantGoods",
	  data: ajaxData,
	  contentType:false,
	  processData: false,
	    success:function(data){
	    	if(data){
	    		$("#revenueReq").append(data);
	    		$("#trLoader").hide();
	    		updateProductRow();
	    		$(".goodSite").select2();
	    	}
