										              ...
										              //  $("#prevHeadsDiv").show();
										                    ...
										                    data.costcenters.map(function(res){
										                     if($("#invoiceMode").val() == 2 || $("#invoiceMode").val() == 3){
										                     		str =`${str}<option value="${res.id}">(${res.site_reference})${res.site_name}(${res.customer_site_ref_no})</option>`;	
													             }else{
...
function checkFinancialYear(checkDay){
		...
		$("#trLoader").show();
		var allData = {mode:$("#pageModeID").val(),checkDate:checkDay,pageMode:$("#pageModeCode").val()}	
		  $.ajax({
                    type: 'GET',
                    url: '/getSerialNumber',
                    data: allData,
...
function initializeGoods(rowsCount){
	  ...
	  var productOptions = {
					...
					getValue: function(element){ 
						if($("#invoiceMode").val() == 2 || $("#invoiceMode").val() == 3){
							return element.label+"- ["+element.goods_balance+"]";
